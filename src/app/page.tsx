...
// Version: 1.7.2-responsive-scaling
...
export default function WeatherPage() {
  ...
  const fetchWeather = async (lat: number, lon: number) => {
    ...
    const cached = localStorage.getItem('weather_cache_v32'); 
    if (cached) {
    ...
    setLoadingStatus('Optimalizujem pixely pre tvoj monitor...');
    setError(null);
    ...
    try {
      ...
      if (aiData.commentaries) {
        ...
        setWeather(fullData);
        localStorage.setItem('weather_cache_v32', JSON.stringify({ lat, lon, timestamp: Date.now(), data: fullData }));
      }
    } catch (err: any) {
  ...
  return (
    <main className="min-h-screen bg-[#020617] text-slate-50 font-sans selection:bg-blue-500/30 overflow-x-hidden pb-24 md:pb-12">
      <div className="max-w-4xl mx-auto p-4 md:p-8 lg:p-12 space-y-4 md:space-y-6 min-h-screen flex flex-col justify-center">
        ...
        {loading ? (
          ...
          <div className="space-y-3 md:space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 md:gap-4">
              ...
              {/* RESPONSIVE HERO BOX */}
              <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-blue-600 to-indigo-700 rounded-[2rem] md:rounded-[2.5rem] p-5 md:p-8 lg:p-10 shadow-2xl relative overflow-hidden min-h-[260px] md:min-h-[300px] flex items-center">
                ...
                <div className="flex w-full items-center justify-between gap-4 md:gap-8 relative z-10">
                  ...
                  <div className="grid grid-cols-2 gap-2 md:gap-3 shrink-0 max-w-[240px] md:max-w-none">
                    {[
                      ...
                      <div key={i} className="bg-white/10 backdrop-blur-md p-2.5 md:p-4 rounded-2xl md:rounded-3xl flex flex-col border border-white/10 shadow-lg transition-transform hover:scale-[1.02]">
                        <s.Icon size={14} className={`${s.color} mb-1 md:mb-2`} />
                        <span className="text-base md:text-xl font-black tabular-nums leading-none mb-0.5">{s.val}</span>
                        <span className="text-[7px] md:text-[9px] font-black uppercase opacity-50 tracking-widest">{s.label}</span>
                      </div>
                    ))}
                  </div>
                  ...
                  <div className="shrink-0 flex flex-col items-end text-right">
                    ...
                    <h2 className="text-[9px] md:text-sm font-black uppercase tracking-widest opacity-90 mt-2 md:mt-4 bg-white/10 px-3 py-1 md:px-4 md:py-1.5 rounded-full backdrop-blur-sm border border-white/5 inline-block">{weather.description}</h2>
                  </div>

                </div>
              </div>

              {/* NEXT DAYS - Side by side on all but tiny screens */}
              <div className="grid grid-cols-2 gap-3 md:gap-4 md:contents">
                <div className="bg-slate-900/80 border border-slate-800 rounded-[2rem] p-4 md:p-6 flex flex-col justify-between hover:border-slate-700 transition-all min-h-[120px] md:min-h-[140px]">
                  ...
                  <div>
                    ...
                    <div className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase mt-1.5 truncate">{weather.tomorrow.description}</div>
                  </div>
                </div>
                ...
                <div className="bg-slate-900/80 border border-slate-800 rounded-[2rem] p-4 md:p-6 flex flex-col justify-between hover:border-slate-700 transition-all min-h-[120px] md:min-h-[140px]">
                  ...
                  <div>
                    ...
                    <div className="text-[8px] md:text-[10px] font-bold text-slate-500 uppercase mt-1.5 truncate">{weather.afterTomorrow.description}</div>
                  </div>
                </div>
              </div>

              {/* AI COMMENTARY */}
              <section className="md:col-span-2 bg-slate-900/40 backdrop-blur-sm border border-slate-800/50 rounded-[2.5rem] p-6 md:p-8 lg:p-10 relative overflow-hidden min-h-[160px] md:min-h-[180px] flex items-center shadow-lg">
        ...
        {/* Action Bar - Fixed on mobile, integrated on desktop */}
        <nav className="fixed bottom-6 left-1/2 -translate-x-1/2 w-[calc(100%-2rem)] max-w-md bg-slate-900/80 backdrop-blur-2xl border border-white/5 rounded-2xl p-1.5 shadow-2xl z-50 md:relative md:bottom-0 md:left-0 md:translate-x-0 md:max-w-none md:bg-transparent md:border-none md:shadow-none md:p-0 md:mt-4">
